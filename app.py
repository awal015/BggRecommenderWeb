...
def recommend_games(user_item_matrix, active_user_id, num_recommendations=10):
    ...
    similar_users = user_similarity_df[active_user_id].sort_values(
        ascending=False)[1:]

    recommended_games = []
    for similar_user, similarity_score in similar_users.items():
        similar_user_ratings = user_item_matrix.loc[similar_user]
        unrated_games = similar_user_ratings[
            user_item_matrix.loc[active_user_id] == 0]
        for game_id, rating in unrated_games.sort_values(
...
def recommend_games_knn(user_item_matrix, active_user_id,
                        num_recommendations=10, k=5):
    """
    ...
    distances, indices = model_knn.kneighbors(
        user_item_matrix.loc[[active_user_id]])
    ...
    for i, similar_user_index in enumerate(indices):
        ...
        for game_id, rating in weighted_ratings.sort_values(
                ascending=False).items():
            if game_id not in recommended_games:
                recommended_games.append(game_id)
            if len(recommended_games) >= num_recommendations:
...
def content_based_recommendations(game_title, tfidf_matrix, games, top_n=10):
    ...
    cosine_similarities = linear_kernel(tfidf_matrix[idx],
...
def content_based_recommendations_cat(game_title, tfidf_matrix, games_cat,
                                      top_n=10):
    """
        ...
        if user_input_type == 'user':
            ...
            if not recommendations:
                return render_template('index.html',
                                       message=f"No recommendations found for user: {user_name}")
            game_names = [
            ...
            if not recommendations:
                return render_template('index.html',
                                       message=f"No recommendations found for game: {game_title}")
            return render_template('index.html',
...
def get_recommendations_json():
    """
    Returns recommendations in JSON format.  Useful for AJAX requests
    ...
    if user_input_type == 'user':
        ...
        recommendations = hybrid_recommendations(
            active_user_id, user_title, user_item_matrix, tfidf_matrix,
            tfidf_matrix_cat, games)
        if not recommendations:
            return jsonify({
